...
async function getUser(id) {
  const [row] = await sql`select id, email, name, avatar_url from users where id = ${id}`;
  return row || null;
}
...
async function guildMembers(guildId) {
  const rows = await sql`
    select u.id, u.name, u.avatar_url, m.nickname, m.joined_at,
           coalesce(
    ...
    group by u.id, u.name, u.avatar_url, m.nickname, m.joined_at
...
async function bansForGuild(guildId) {
  return sql`
    select b.user_id, b.reason, b.banned_by, b.created_at, u.name, u.avatar_url
...
module.exports = {
  ...
  upsertUser, getUser,
  createGuild, getGuild, guildsForUser, renameGuild, deleteGuild, transferOwnership,
